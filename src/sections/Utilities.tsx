...
const COMPACT_UTILITIES = [
  { name: 'Privacy Controls', icon: Lock, color: 'text-emerald-400', bg: 'bg-emerald-400/10' },
  { name: 'Global Search', icon: Search, color: 'text-slate-400', bg: 'bg-slate-400/10' },
  { name: 'Announcements', icon: Megaphone, color: 'text-orange-400', bg: 'bg-orange-400/10' },
  { name: 'Event Branding', icon: Palette, color: 'text-pink-400', bg: 'bg-pink-400/10' },
  { name: 'Social Sharing', icon: Share2, color: 'text-blue-500', bg: 'bg-blue-500/10' },
  { name: 'Push Alerts', icon: Bell, color: 'text-amber-400', bg: 'bg-amber-400/10' },
];

export const Utilities = () => {
  return (
    <section id="utilities" className="py-32 relative overflow-hidden noise-overlay bg-white dark:bg-[#0b101a]">
      ...
      <Container className="relative z-10">
          ...
          {FEATURED_UTILITIES.map((item, i) => (
            <motion.div
              ...
              className={`${item.size} group relative rounded-[2rem] p-8 overflow-hidden
                bg-white/60 dark:bg-[#0d1525]/80
                border border-slate-200/50 dark:border-white/[0.06]
                backdrop-blur-xl transition-all duration-500
                hover:-translate-y-1 ${item.glow}
                shadow-[0_8px_30px_rgb(0,0,0,0.02)] dark:shadow-[0_20px_50px_rgba(0,0,0,0.3)]`}
            ...
            >
              <div className="relative z-10">
                ...
                <h3 className="text-2xl font-extrabold text-brand-navy dark:text-white mb-3 tracking-tight">
                  {item.name}
                </h3>
              ...
              <div className="absolute -bottom-10 -right-10 w-40 h-40 bg-gradient-to-br from-brand-coral/10 to-transparent blur-3xl opacity-0 group-hover:opacity-100 transition-opacity duration-700" />
          ...
          {SECONDARY_UTILITIES.map((item, i) => (
            <motion.div
              ...
              className={`${item.size} group relative rounded-[1.75rem] p-6 overflow-hidden
                ...
                border border-slate-200/50 dark:border-white/[0.06]
                backdrop-blur-xl transition-all duration-500
                hover:-translate-y-1 hover:shadow-[0_15px_35px_rgba(0,0,0,0.05)]
            ...
            >
              <div className="relative z-10">
                ...
                <h4 className="text-lg font-bold text-brand-navy dark:text-white mb-2 tracking-tight">
                  {item.name}
                </h4>
            ...
            {COMPACT_UTILITIES.map((item, i) => (
              <motion.div
                ...
                className="group flex items-center gap-3 p-4 rounded-2xl
                  bg-white/40 dark:bg-[#0d1525]/40
                  border border-slate-200/40 dark:border-white/[0.04]
                  backdrop-blur-md transition-all duration-300
                  hover:bg-white/60 dark:hover:bg-[#141e35]/60
              ...
              >
                ...
                <span className="text-[11px] font-bold text-brand-navy dark:text-grey-300 tracking-tight leading-tight">
                  {item.name}
                </span>
              </motion.div>
            ))}
        ...
        >
          <div className="inline-flex items-center gap-2 px-4 py-2 rounded-full
            bg-brand-coral/[0.05] dark:bg-brand-coral/10 border border-brand-coral/20
            text-brand-coral text-xs font-bold tracking-wide">
            <Sparkles className="w-3.5 h-3.5" />
            Discover 50+ additional features inside the app
          </div>
        </motion.div>
      </Container>
    </section>
