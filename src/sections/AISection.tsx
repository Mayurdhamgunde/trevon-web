...
export const AISection = () => {
  ...
  return (
    <section id="ai" className="py-24 relative overflow-hidden" ref={ref}>
      {/* Background wash */}
      <div className="absolute inset-0 bg-gradient-to-b from-transparent via-brand-coral/5 dark:via-brand-coral/8 to-transparent pointer-events-none" />
      ...
      <Container>
        ...
        >
          <Badge variant="subtle" className="mb-4">TrevAI Assistant</Badge>
          <h2 className="text-4xl md:text-5xl lg:text-6xl font-extrabold text-brand-navy dark:text-white mb-5 leading-tight">
            Watch AI plan your trip<br />
            <span className="text-transparent bg-clip-text bg-gradient-to-r from-brand-coral to-brand-orange">
              in real time.
            </span>
          </h2>
          <p className="text-lg text-grey-500 dark:text-grey-400 max-w-xl mx-auto">
            One sentence is all it takes. TrevAI builds a complete plan — schedule, budget, team, venues — instantly.
          </p>
        </motion.div>

        {/* Workspace card */}
        <div className="glass-card rounded-[2.5rem] relative overflow-hidden">
          {/* Orbs */}
          <div className="absolute -top-32 -right-32 w-96 h-96 bg-brand-coral/15 rounded-full blur-3xl pointer-events-none animate-orb-drift" />
          <div className="absolute -bottom-32 -left-32 w-96 h-96 bg-brand-orange/15 rounded-full blur-3xl pointer-events-none animate-orb-drift-r" />
          ...
          <div className="relative z-10 p-6 md:p-10">
            ...
            <div className="grid grid-cols-1 lg:grid-cols-[1fr_1.5fr_1fr] gap-5">
              ...
              <div className="flex flex-col gap-4">
                {/* Prompt bubble */}
                <motion.div
                  className="rounded-2xl p-4 bg-white/50 dark:bg-white/[0.04] border border-slate-200/40 dark:border-white/8 backdrop-blur-sm"
                ...
                >
                  <div className="flex gap-3">
                    ...
                    <p className="text-sm text-brand-navy dark:text-grey-200 leading-relaxed min-h-[2.5rem]">
                      {promptText}
                      {!promptDone && started && (
                ...
                <AnimatePresence>
                  {promptDone && (
                    <motion.div
                      className="rounded-2xl p-4 bg-brand-coral/5 dark:bg-brand-coral/8 border border-brand-coral/20"
                      initial={{ opacity: 0, y: 10 }}
                      animate={{ opacity: 1, y: 0 }}
                      exit={{ opacity: 0 }}
              ...
              {/* ── CENTER: Itinerary ── */}
              <motion.div
                className="rounded-2xl p-5 bg-white/50 dark:bg-white/[0.04] border border-slate-200/40 dark:border-white/8 backdrop-blur-sm"
                ...
                <div className="flex flex-col gap-3">
                  {steps.map((step, i) => (
                    <AnimatePresence key={i}>
                      {showPanels && (
                        <motion.div
                          className="flex items-start gap-3 p-3 rounded-xl bg-white/60 dark:bg-white/[0.03] border border-slate-200/30 dark:border-white/5"
                        ...
                        >
                          ...
                          <div className="flex-1 min-w-0">
                            <div className="flex items-center justify-between gap-2 mb-0.5">
                              <span className="text-xs font-bold text-brand-coral uppercase tracking-wide">{step.day}</span>
                              <span className={`text-[10px] px-1.5 py-0.5 rounded-full font-semibold ${step.ok ? 'bg-emerald-400/10 text-emerald-400' : 'bg-yellow-400/10 text-yellow-400'}`}>
                                {step.ok ? 'confirmed' : 'pending'}
                              </span>
                            </div>
                            <p className="text-xs font-semibold text-brand-navy dark:text-white truncate">{step.title}</p>
                            <div className="flex items-center gap-1 mt-1">
                              <Clock className="w-3 h-3 text-grey-400" />
                              <span className="text-[10px] text-grey-400">{step.time}</span>
                            </div>
                          </div>
                        </motion.div>
                      )}
            ...
            {/* Bottom prompt bar */}
            <motion.div
              className="mt-6 flex items-center gap-3 rounded-2xl px-4 py-3 bg-white/50 dark:bg-white/[0.04] border border-slate-200/40 dark:border-white/8 backdrop-blur-sm"
